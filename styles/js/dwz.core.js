...
(function($){
	...
	window.DWZ = {
		version: 'dwz for bootstrap v1.0.0',
		...
		eventType: {
			pageClear:'dwz_pageClear',	// 用于重新ajaxLoad，去除xheditor等需要特殊处理的资源
			ajaxDone:'dwz_ajaxDone', // ajax 表单提交完成触发的事件
			dialogAjaxSuccess:'dwz_dialogAjaxSuccess'
		},
		...
		globalLoging:{
			...
			show:function(){
				...
				if (window.currentLoadingBtn) { // data-loading-text
					window.currentLoadingBtn.button('loading');
				} else {
					$('#'+DWZ.conf.backgroundId).height($(document).height());
					var $ajaxBg = $('#'+DWZ.conf.progressBarId+', #'+DWZ.conf.backgroundId).show();

					setTimeout(function(){
						if ($ajaxBg.is(':visible')) {$ajaxBg.hide();}
					}, 5000);
				}
			},
			hide:function(){
				...
				if (window.currentLoadingBtn) { // data-loading-text
					window.currentLoadingBtn.button('reset');
					window.currentLoadingBtn = null;
				} else {
					$('#'+DWZ.conf.progressBarId+', #'+DWZ.conf.backgroundId).hide();
				}
			}
		},
		alert:{
	...
	$.fn.extend({
		...
		ajaxUrl: function(op){
			...
			$.ajax({
				...
				complete: function(xhr, ts){
					if ($.isFunction(op.completeFn)) op.completeFn();
				},
				statusCode: {
		...
		isBind:function(type) {
			var _events = this.data("events") || $._data(this[0],'events');
